...
RandomParkMiller::RandomParkMiller(unsigned long Dimensionality_, 
	unsigned long Seed): RandomBase(Dimensionality_), 
...
void RandomParkMiller::GetUniforms(vector<double>& variates){
	...
	{
		temp = InnerGenerator.GetOneRandomInteger()*Reciprocal;
		variates[j] =  temp;
	}
}

void RandomParkMiller::Skip(unsigned long numberOfPaths){
	vector<double> tmp (0.0,GetDimensionality());   
